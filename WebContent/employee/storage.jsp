					...
					<div>
						<div class="row">
							<div class=" col-lg-12 col-xs-12"
								...
								<input placeholder="请输入商品IBSN码或商品名称" type="text" id="keyword"
...
<script src="scripts/bootstrap.min.js"></script> END JAVASCRIPTS -->
	...
	$(document).ready(function(){
		...
		$.getJSON("../List",function(data){
			...
			html += "<td><a href=\"modifyStorage.jsp?productID="
							+ data[i].productID
						+"\"  >更新库存  </a></td></tr>";
			}
			$("table").append(html);		
   ...
   $(document).ready(function() {
		$("#sousuo").click(function() {
			$.getJSON("../searchstorage",{
				keyword : $("#keyword").val()},
				function(jsonData) {
						$("table tr:gt(0)").remove();
								var html = "";
							for (var i=0; i < jsonData.length; i++) {
									html += "<tr><td>" + jsonData[i].productID + "</td>";
									html += "<td>" + jsonData[i].name+ "</td>";
									html += "<td>" + jsonData[i].amount + "</td>";
									html += "<td>" + jsonData[i].updatetime + "</td>";
									html += "<td><a href=\"modifyStorage.jsp?productID="
